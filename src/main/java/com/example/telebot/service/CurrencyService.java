...
import com.example.telebot.model.CurrencyModel;
...
public class CurrencyService {
    ...
    public static String getCurrencyRate(String message, CurrencyModel model) throws IOException, ParseException {
        ...
        CurrencyModel rubModel = new CurrencyModel();
        URL rubUrl = new URL("https://www.nbrb.by/api/exrates/rates/RUB?parammode=2");
        scanner = new Scanner((InputStream) rubUrl.getContent());
        StringBuilder rubResult = new StringBuilder();
        while (scanner.hasNext()){
            rubResult.append(scanner.nextLine());
        }
        JSONObject rubObject = new JSONObject(rubResult.toString());

        rubModel.setCur_OfficialRate(rubObject.getDouble("Cur_OfficialRate"));
        rubModel.setCur_Scale(rubObject.getInt("Cur_Scale"));
        ...
        return "Official rate of BYN to " + model.getCur_Abbreviation() + "\n" +
                ...
                "\n\n" +
                "Official rate of RUB to " + model.getCur_Abbreviation() + "\n" +
                "is: " + String.format("%.2f", rubModel.getCur_Scale() / rubModel.getCur_OfficialRate() * model.getCur_OfficialRate()) +
                " RUB per " + model.getCur_Scale() + " " + model.getCur_Abbreviation();
